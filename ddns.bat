...
wmic ENVIRONMENT where "name='dns_record_update'" get UserName,VariableValue |findstr SYSTEM > dns_record_update.env
for /f  "tokens=2 delims=<>	 " %%i in (dns_record_update.env) do set dns_record_update=%%i
...
cscript /nologo %~dp0ddns.vbs /r reclist.txt
::del reclist.txt
goto end
...
rem for /f "delims=<> tokens=2 " %%i in ('curl -X POST https://dnsapi.cn/Domain.List -d "login_token=%dns_pod_token%&format=XML" --silent ^|findstr "<id>.*</id> <name>.*</name>" ') DO echo %%i >> domainlist.txt
...
for /f "tokens=1,3,5 delims=：— " %%i in ( 'cscript /nologo %~dp0ddns.vbs /D  domainlist.txt') do echo 域名 %%i：%%j：%%k & set dns_pod_domain_ID%%i=%%j & set dns_pod_domain_name%%i=%%k & set dd_total=%%i
...
cscript /nologo %~dp0ddns.vbs /r reclist.txt > recclist.txt
del reclist.txt
type recclist.txt
